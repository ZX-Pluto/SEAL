...
// Ciphertext my_for(Evaluator evaluator, Ciphertext temp, Ciphertext c1, RelinKeys relin_keys){
//     for (size_t i = 0; i < 100; i++)
//     {
//         evaluator.square(c1, temp);
...
void perf_test()
{
    ...
    */
    ...
    {
        temp = x3_encrypted;
        evaluator.multiply_inplace(temp, x1_encrypted_coeff3);
        // evaluator.square(x1_encrypted, temp);
        evaluator.relinearize_inplace(temp, relin_keys);
        evaluator.rescale_to_next_inplace(temp);
    }
    // return temp;

    // temp = my_for(evaluator, temp, x1_encrypted, relin_keys);
    ...
    */
    ...
    cout << "    + Modulus chain index for temp_encrypted: "
